...
//
// 捕获 (resp_p5_udp_split.memh): 每拍 "%02h %d" (gmii_txd, tx_en) + 事件行
//   FEND/ACK; 末尾统计行 (见 initial 尾部): P5EV 事件字 / P5REG 寄存器快照 /
//   P5TX / P5INV / STATS_/TCBF/ECOMAX 等。
//
// ---- P5c-T4: `P5_CLOSE` 关闭语义门 (G7 门基础设施) ----
// 同一 TB 的第二个用例 (`run_tb_p5_app.bat close`), 独立分支 + 独立落盘文件
// (resp_p5_udp_split_close.memh), 默认路径逐字不变:
//   · PC 模型**正确处理我方 FIN**: 收到 flags=0x11 的帧按"ack = FIN.seq + 1"
//     回纯 ACK (照抄 tb_p5_adv.v 的 FIN-ACK 逻辑, L870-889); 另有"丢 FIN"旋钮
//     (fin_drop_rem) 模拟 FIN 丢失 ⇒ 板侧只能靠 RTO 重发。
//   · **能注入对端 FIN**: flags=0x11 的 60B 帧 (inj_arm 的 fin 参数)。注意
//     rx_classify 把带 FIN 的 TCP 帧路由到慢路径 (rtl/rx_classify.v:71), 本门
//     慢口被排空 ⇒ 再注入一条 CFG DEL 记录模拟 HLS 的反应 (state=0 + CAM 清 +
//     ev_down)。
//   · 阶段化驱动 (cfg ADD/DEL + CMD close/abort + PC 注入), 每阶段独立 seq 基,
//     逐阶段判据在 tools/gen_stim_p5_app.py 的 close 分支。
//   · 计时器缩放: RTO_LIM/FIN_TO_LIM 由 TB_* localparam 覆盖 (生产值 12.5M/50M
//     拍在 xsim 里跑不动, 与 sim/p5close 的做法一致); **比例保持 4xRTO**, 生产值
//     由 checker 从 rtl/ 源码断言 (绝不放宽: 缩放只换计时器长短, 不换代码路径)。
//=============================================================================
module tb_p5_udp_split;
`ifdef P5_CLOSE
    localparam [31:0] TB_TX_BYTES = 32'd35040;      // close 门: 24 帧 x 1460B/阶段
`elsif P5_SMALL
...
`ifdef P5_CLOSE
    // ---- 关闭门参数 ----
    // 每阶段独立 snd 基 (帧归属按 tick 窗 + seq 双确认; 见 checker)
    localparam [31:0] PH_A_BASE  = 32'h12345679;    // ① 干净关闭
    localparam [31:0] PH_B_BASE  = 32'h22345679;    // ② FIN 丢失->RTO 重发 + ⑤ 关闭超时
    localparam [31:0] PH_D1_BASE = 32'h32345679;    // ④-a 同时关闭 (我方 FIN 先)
    localparam [31:0] PH_D2_BASE = 32'h42345679;    // ④-b 同时关闭 (DEL 先 => 0 FIN)
    localparam [31:0] PH_E_BASE  = 32'h52345679;    // (B 已含超时; E 保留给超时后重连前的净态)
    localparam [31:0] PH_F_BASE  = 32'h62345679;    // ⑤ 新连接拿到非零 rcv_wnd
    localparam [31:0] PH_C_BASE  = 32'h72345679;    // ③ abort -> RST + state=0 (末阶段)
    localparam [31:0] PH_G_BASE  = 32'h82345679;    // ③b abort fence 探针 (RST 早于数据)
    localparam [15:0] PH_NFRAMES = 16'd24;          // 每阶段 app 数据帧数 (35040/1460)
    localparam        TB_AUTO_CLOSE = 1'b0;         // close 门由 TB 显式 CMD 关闭
`ifdef P5_CLOSE_REAL
    // 生产方式计时 (12.5M + 50M 拍, 一次跑约 1 小时; 仅在 P5_CLOSE_REAL 下)
    localparam [31:0] TB_RTO_LIM    = 32'd48828;
    localparam [17:0] TB_FIN_TO_LIM = 18'd195313;
`else
    // 缩放: RTO = 3x256 = 768 拍; 超时 = 12x256 = 3072 拍 = **4 x RTO** (比例与生产一致)
    localparam [31:0] TB_RTO_LIM    = 32'd3;
    localparam [17:0] TB_FIN_TO_LIM = 18'd12;
`endif
    localparam [17:0] TB_FIN_GRACE  = 18'd4;        // RST 宽限轮数 (同 RTL 默认)
    localparam [31:0] TB_RTO_TICKS  = TB_RTO_LIM * 32'd256;   // 一个 RTO 的拍数
`else
    // 默认/flow 构建: 逐字回退到 RTL 默认值 (行为与既有门完全一致)
    localparam        TB_AUTO_CLOSE = 1'b1;
    localparam [31:0] TB_RTO_LIM    = 32'd48828;
    localparam [17:0] TB_FIN_TO_LIM = 18'd195313;
    localparam [17:0] TB_FIN_GRACE  = 18'd4;
`endif
...
`endif
    ...
    rx_classify u_classify (
        ...
        // P5e 前置实验: 原 1'b1 (排空) 换成 udp_rx 的 s_axis_tready (零 RTL 改动)
    ...
    tcp_tx_frame #(
        // P5c-T4: close 门用缩放 RTO (默认构建 = RTL 默认值 48828, 逐位不变)
        .RTO_LIM(TB_RTO_LIM)
    ) u_tx (
        .clk(clk), .rst_n(rst_n),
...
`ifdef P5_FLOW
    ...
    assign pat_led        = 4'd0;
`else
    app_pattern #(.TX_BYTES(TB_TX_BYTES), .AUTO_CLOSE(TB_AUTO_CLOSE)) u_app (
        .clk(clk), .rst_n(rst_n),
...
`ifdef P5_FLOW
    ...
    // P5b 诊断 (第二轮): 独立参考 LFSR —— 与 psnd_nxt/pretx_seq 严格同步推进,
    // 与发送流用的 plfsr/plfsr_rt 对账。一旦不等 ⇒ 对端自身发的载荷内容错位
    // (而 seq 是对的) ⇒ sink 侧表现为"局部 256B 失配后自动对齐"。纯观测。
    reg  [63:0] pref, pref_rt;
    ...
    task flow_build;
        ...
        begin
            ...
            // ⚠️ P5b 第二轮 TB 修复: 本任务**不再**更新图案 LFSR。
            // 旧写法无条件写 `plfsr <= tls; plfsr_rt <= tls;` —— 但本任务既服务
            // 正常流 (s0 = plfsr) 又服务重传流 (s0 = plfsr_rt), 于是每次重传都把
            // **正常流**的 LFSR 拽到重传位置 + slen ⇒ 之后正常段 seq 正确而载荷
            // 内容整体错位 (sink 侧表现为局部图案失配后自动对齐)。第一版没暴露
            // 是因为"只发整段"绕行下 peer_retx 恒 0 (重传路径从未跑过)。
            // 现在由**调用方**各自推进自己的流 (与 s0 严格同步):
            //   正常: plfsr    <= xs_adv(plfsr,    flow_slen);
            //   重传: plfsr_rt <= xs_adv(plfsr_rt, flow_slen_rt);
            // 并有独立参考 LFSR (pref/pref_rt) 对账 (PATMM/PATMM_RT 诊断打印)。
        end
    endtask
    ...
    // 同上, 但写独立参考 (诊断用: 与 plfsr_rt 对账 ⇒ 判"对端自产内容是否错位")
    task flow_seek_pref;
        input [31:0] target;
        integer      n;
        reg   [63:0] t;
    ...
    // 发送门: 窗口有余量 (>0) + 连接已建 ⇒ 就发 (段长由 flow_slen 按窗口/总量钳位)。
    // ⚠️ P5b 第二轮 C20: 这里**撤销**了第一版的"只发整段"绕行 (原式要求
    // `flow_room >= 1460`, 窗口吃紧时对端根本不发 ⇒ flow 门从未进入窗口吃紧区 ⇒
    // 判据④是靠"永不进入"满足的, 是假绿)。真实 TCP 发送端在窗口只剩零头时**照样
    // 发零头段** (SWS 规避只约束"小窗通知", 不禁止按窗口余量发短段), 且 P5b 的
    // 动态窗口收缩正是靠这种零头段暴露问题 ⇒ 必须覆盖。
    // 第一版那段注释把原因归到 "tcp_rx 的 pcount 泄漏" —— **归因错误, 已证伪**:
    // `state <= S_PAY` 全文件唯一且在入口同拍无条件写 pcount 初值 (tcp_rx.v),
    // 独立复核统计 84 次进入 S_PAY 全部 pcount=2。真因是 `tcp_rx` 的 w6 拒绝路径
    // (wcnt==6 判定拍): 对端按**旧窗口**发的零头段落地时 `ra_rcv_wnd` 已被扫描写到
    // 0 (加 C1b 的 Δ 漂移) ⇒ `seq_eq && !win_ok` ⇒ 静默丢弃 (既无 fend 也无 ACK)。
    // 这正是 C16/C16-修订要修的工况 (ACC_MARGIN 让它在裕度内被接受 ⇒ 不再丢)。
    // 中流短段现在是**显式用例** (不再是绕行): 判据期待"零重传 + 逐字节完整"。
    ...
    always @(posedge clk) begin
        if (!rst_n) dseq_r <= 32'd0;
            ...
            if ((rx_stat_seq != dseq_r) && (rx_stat_seq != 32'd0))
                $display("DROP k=%0d rcv_nxt=%08h wnd=%04x occ=%0d redge=%08h right=%08h praw=%08h psnd=%08h lastpraw_ack=%08h | acc_l=%0d ackr_l=%0d w6a_l=%0d seq32=%08h rn_l=%08h plen=%0d wcnt=%0d st=%0d dif=%0d",
                         ...
                         u_rx.acc_l, u_rx.ackresp_l, u_rx.w6a_ok_l, u_rx.seq32_l,
                         u_rx.rcv_nxt_l, u_rx.plen_l, u_rx.wcnt, u_rx.state,
                         (u_rx.seq32_l - u_rx.rcv_nxt_l));
        end
    end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (flow_retx_go) begin
                ...
                flow_seek_pref(puna);   // 独立参考 (诊断: 与 plfsr_rt 对账)
...
`endif
    ...
    app_ctrl #(.WIN_CAP(16'hBFFE),
               // P5c-T4: close 门缩放关闭超时 (默认构建 = RTL 默认值, 逐位不变)
               .FIN_TO_LIM(TB_FIN_TO_LIM), .FIN_GRACE(TB_FIN_GRACE)) u_app_ctrl (
        .clk(clk), .rst_n(rst_n),
        .ev_up(ev_up), .ev_down(ev_down), .ev_slot(ev_slot),
        .ev_peer_ip(ev_peer_ip), .ev_peer_port(ev_peer_port),
        .ev_peer_mac(ev_peer_mac),
    ...
    reg        tx_en_d, tx_inf;
    reg [5:0]  txbc;
    // P5c-T4: 捕获窗口 48 -> 56 字节 (close 门要读 window 字段 cap[48:49]);
    // cap[0:47] 的取值与既有门逐位相同 (没有任何读口越过 47)
    reg [7:0]  cap [0:55];
`ifdef P5_CLOSE
    // ---- close 门: 帧分类 / PC 模型 FIN-ACK / 阶段驱动 (声明须在用到它的
    //      always 块之前; 见文件头 P5_CLOSE 说明) ----
    wire       conn_any = (txbc >= 6'd48) && cap[12] == 8'h08 && cap[13] == 8'h00 &&
                          cap[23] == 8'h06 && cap[34] == 8'h1F && cap[35] == 8'h90 &&
                          cap[36] == 8'h30 && cap[37] == 8'h39;
    reg [15:0] frm_fin_n;        // 线上 FIN 帧数
    reg [15:0] frm_rst_n;        // 线上 RST 帧数
    reg [15:0] frm_oth_n;        // 线上其它 TCP 帧数 (纯 ACK 等)
    reg [15:0] fin_drop_rem;     // 还要"丢"几个 FIN 的 ACK (模拟 FIN 丢失)
    reg [15:0] fin_acked_n;      // 已被 PC 回 ACK 的 FIN 数
    reg [31:0] fin_seq0;         // 首个 FIN 的 seq
    reg [31:0] fin_seq_last;     // 最近一个 FIN 的 seq
    reg [31:0] ph_base;          // 本阶段 snd 基
    reg [3:0]  ph_id;
    reg [31:0] ph_d0, ph_f0, ph_r0, ph_e0, ph_t0;   // 阶段基线
    reg [15:0] ph_s0;                               // 阶段 state0 计数基线
    reg [31:0] ph_ab0, ph_af0;                      // 阶段 app 字节/帧数基线
    reg [31:0] ph_d, ph_f, ph_r, ph_e;              // 阶段增量
    reg [15:0] st0_n;            // TCB.state[0] 1->0 次数 (只增, 阶段基线做差)
    reg [3:0]  st0_q;
    reg [15:0] finreq_n;         // app_ctrl.fin_req[0] 上升沿次数
    reg        finreq_q;
    reg [15:0] rstreq_n;         // app_ctrl.rst_req[0] 上升沿次数
    reg        rstreq_q;
    reg [31:0] tmo_vec;          // 等待超时位图 (0 = 全部正常)
    reg        tx_en_p;
    reg [31:0] frm_t0;
    integer    frm_idx;
    integer    wt;
`endif
    ...
    function [7:0] inj_byte;
        ...
        input        finj;     // P5c-T4: 1 = 对端 FIN 段 (flags 0x11)
        ...
        begin
            case (idx)
                ...
                7'd47: inj_byte = finj ? 8'h11 : (isd ? 8'h18 : 8'h10);
    ...
    //              P5c-T4: finj=1 -> 对端 FIN 段 = 60B + flags 0x11)
    task inj_arm;
        input        isd;
        input        finj;     // P5c-T4: 1 = FIN 段 (flags 0x11, 60B)
        input [31:0] sq;
        input [31:0] ak;
        reg [8:0]  blen;
        reg [15:0] tot;
            ...
            for (jj = 0; jj < 200; jj = jj + 1) begin
                if (jj < blen) begin
                    if (isd && jj >= 54) begin
                        ...
                        bb = inj_byte(jj[6:0], sq, ak, ip_csum_inj(tot), isd,
                                      finj, 8'd0);
                    end
                    inj_buf[8 + jj] <= bb;
...
`ifdef P5_FLOW
            ...
            if (flow_play) begin
                ...
                if (plfsr !== pref)
                    $display("PATMM k=%0d seq=%08h plfsr=%016h pref=%016h",
                             k, psnd_nxt, plfsr, pref);
                flow_build(psnd_nxt, flow_slen, plfsr);
                plfsr <= xs_adv(plfsr, flow_slen);   // 正常流自身推进 (见 flow_build 注)
                pref  <= xs_adv(pref,  flow_slen);
                ...
                if (plfsr_rt !== pref_rt)
                    $display("PATMM_RT k=%0d seq=%08h plfsr_rt=%016h pref_rt=%016h",
                             k, pretx_seq, plfsr_rt, pref_rt);
                flow_build(pretx_seq, flow_slen_rt, plfsr_rt);
                plfsr_rt <= xs_adv(plfsr_rt, flow_slen_rt);   // 重传流自身推进
                pref_rt  <= xs_adv(pref_rt,  flow_slen_rt);
...
`else
            if (inj_play) begin
                ...
                if (rxdata_go) begin
                    ...
                    inj_arm(1'b1, 1'b0, u_tcb.rcv_nxt_r[0], hi_wm);
                    rxdata_go <= 1'b0;
                    rxdata_done <= 1'b1;
                end else begin
                    seq_b = u_tcb.rcv_nxt_r[0];
                    ack_b = inj_ack_val;
                    inj_arm(1'b0, 1'b0, seq_b, ack_b);
                    inj_done <= echo_seen;
                end
                rx_dv <= 1'b0;
                rx_er <= 1'b0;
...
`ifdef P5_CLOSE
            // P5c-T4: FIN/RST 帧分类 + **FIN-ACK** (照抄 tb_p5_adv.v L870-889:
            // 收到 flags=0x11 的帧按 "ack = FIN.seq + 1" 回一条纯 ACK)。数据帧
            // 依旧走上面的 conn_d0 分支 (与默认门**共用同一段** ACK 逻辑)。
            else if (tx_en_d && conn_any) begin
                if (cap[47] == 8'h11) begin
                    frm_fin_n    <= frm_fin_n + 16'd1;
                    fin_seq_last <= {cap[38], cap[39], cap[40], cap[41]};
                    if (frm_fin_n == 16'd0)
                        fin_seq0 <= {cap[38], cap[39], cap[40], cap[41]};
                    if (fin_drop_rem != 16'd0) begin
                        // "丢包": 不回 ACK ⇒ 板侧只能靠 RTO 重发 (判据 ②)
                        fin_drop_rem <= fin_drop_rem - 16'd1;
                    end else begin
                        echo_seen   <= echo_seen + 16'd1;      // 触发一次 ACK 注入
                        inj_ack_val <= {cap[38], cap[39], cap[40], cap[41]} + 32'd1;
                        fin_acked_n <= fin_acked_n + 16'd1;
                    end
                end else if (cap[47] == 8'h14) begin
                    frm_rst_n <= frm_rst_n + 16'd1;            // RST 不 ACK
                end else begin
                    frm_oth_n <= frm_oth_n + 16'd1;
                end
            end
`endif
        end
...
`ifdef P5_CLOSE
    // ==========================================================================
    // P5c-T4 close 门 (G7 门基础设施) — 轨迹落盘 + 阶段化关闭语义驱动
    //
    // 判据全部在 tools/gen_stim_p5_app.py 的 close 分支 (TB 只产生激励与证据):
    //   P5FRM 每帧一条 (idx 与字节流落盘的帧序一一对应 ⇒ checker 交叉校验字段)
    //   P5EVT 关键时刻拍号 (FINREQ / RSTREQ / ST0 / EVDOWN)
    //   P5PH  阶段边界 (id/base/各计数增量/起止拍)
    //   P5REGP 阶段寄存器快照 (与默认门的 P5REG 分开, 后者按地址去重)
    // ==========================================================================
    always @(posedge clk) begin
        if (!rst_n) begin
            tx_en_p <= 1'b0; frm_t0 <= 32'd0; frm_idx <= 0;
        end else begin
            tx_en_p <= gmii_tx_en;
            if (gmii_tx_en && !tx_en_p) frm_t0 <= k;
            if (!gmii_tx_en && tx_en_p) begin
                $fwrite(fd, "P5FRM %0d %0d %0d %02h %02h %02h %02h %08h %08h %04h %04h %012h %012h %04h %04h\n",
                        frm_idx, frm_t0, k, cap[12], cap[13], cap[23], cap[47],
                        {cap[38], cap[39], cap[40], cap[41]},
                        {cap[42], cap[43], cap[44], cap[45]},
                        {cap[48], cap[49]}, {cap[16], cap[17]},
    ...
    reg fin_sent_q, rst_sent_q;
    always @(posedge clk) begin
        if (!rst_n) begin
            fin_sent_q <= 1'b0; rst_sent_q <= 1'b0;
        end else begin
            // 帧器完成拍 (S_DONE 置 fin_sent_r/rst_sent_r) —— 与线上帧首可能有
            // mac_tx 内部流水差 (~60 拍): T3 的时序契约以**发出**为准 (state=0
            // 必须晚于它), 线上帧的存在性由帧字节流单独断言。
            fin_sent_q <= tx_fin_sent[0];
            if (!fin_sent_q && tx_fin_sent[0]) $fwrite(fd, "P5EVT FINSENT %0d\n", k);
            rst_sent_q <= tx_rst_sent[0];
            if (!rst_sent_q && tx_rst_sent[0]) $fwrite(fd, "P5EVT RSTSENT %0d\n", k);
        end
    end

    always @(posedge clk) begin
        if (!rst_n) begin
            st0_q <= 4'd0; st0_n <= 16'd0;
            finreq_q <= 1'b0; finreq_n <= 16'd0;
            rstreq_q <= 1'b0; rstreq_n <= 16'd0;
        end else begin
            st0_q <= u_tcb.state_r[0];
            if ((st0_q != 4'd0) && (u_tcb.state_r[0] == 4'd0)) begin
                st0_n <= st0_n + 16'd1;
                $fwrite(fd, "P5EVT ST0 %0d\n", k);
            end
            finreq_q <= u_app_ctrl.fin_req[0];
            if (!finreq_q && u_app_ctrl.fin_req[0]) begin
                finreq_n <= finreq_n + 16'd1;
                $fwrite(fd, "P5EVT FINREQ %0d\n", k);
            end
            rstreq_q <= u_app_ctrl.rst_req[0];
            if (!rstreq_q && u_app_ctrl.rst_req[0]) begin
                rstreq_n <= rstreq_n + 16'd1;
                $fwrite(fd, "P5EVT RSTREQ %0d\n", k);
            end
            if (ac_ev_down) $fwrite(fd, "P5EVT EVDOWN %0d %0d\n", k, ac_ev_slot);
        end
    end

    // ---- 等待原语 (全部带硬上限; 超时**不中断**驱动, 只设 tmo 位由 checker 判) ----
    task wait_tk;
        input [31:0] n;
        begin
    ...
    task wait_dfr;               // 线上数据帧增量 >= n
        input [15:0] n;
        begin
            wt = 0;
            while (((frm_seen - ph_d0[15:0]) < n) && (wt < 400000)) begin
                @(posedge clk); wt = wt + 1;
            end
            if ((frm_seen - ph_d0[15:0]) < n) tmo_vec[0] = 1'b1;
        end
    endtask

    task wait_dff;               // 线上 FIN 帧增量 >= n
        input [15:0] n;
        begin
            wt = 0;
            while (((frm_fin_n - ph_f0[15:0]) < n) && (wt < 400000)) begin
                @(posedge clk); wt = wt + 1;
            end
            if ((frm_fin_n - ph_f0[15:0]) < n) tmo_vec[1] = 1'b1;
        end
    endtask

    task wait_dfrst;             // 线上 RST 帧增量 >= n
        input [15:0] n;
        begin
            wt = 0;
            while (((frm_rst_n - ph_r0[15:0]) < n) && (wt < 400000)) begin
                @(posedge clk); wt = wt + 1;
            end
            if ((frm_rst_n - ph_r0[15:0]) < n) tmo_vec[2] = 1'b1;
        end
    endtask

    task wait_devdown;           // CONN_DOWN 计数增量 >= n
        input [15:0] n;
        begin
            wt = 0;
            while (((ac_ev_down_c[15:0] - ph_e0[15:0]) < n) && (wt < 400000)) begin
                @(posedge clk); wt = wt + 1;
            end
            if ((ac_ev_down_c[15:0] - ph_e0[15:0]) < n) tmo_vec[3] = 1'b1;
        end
    endtask

    task wait_state0;            // TCB.state[0] 1->0 次数增量 >= n
        input [15:0] n;
        begin
            wt = 0;
            while (((st0_n - ph_s0) < n) && (wt < 400000)) begin
                @(posedge clk); wt = wt + 1;
            end
            if ((st0_n - ph_s0) < n) tmo_vec[4] = 1'b1;
        end
    endtask

    task wait_idle;              // snd_nxt == snd_una (在飞数据全部被 ACK)
        begin
            wt = 0;
            while ((u_tcb.snd_nxt_r[0] != u_tcb.snd_una_r[0]) && (wt < 400000)) begin
                @(posedge clk); wt = wt + 1;
            end
            if (u_tcb.snd_nxt_r[0] != u_tcb.snd_una_r[0]) tmo_vec[5] = 1'b1;
        end
    endtask

    task wait_una_eq;            // snd_una == s (判 "对端 ACK 了 FIN")
        input [31:0] s;
        begin
            wt = 0;
            while ((u_tcb.snd_una_r[0] != s) && (wt < 400000)) begin
                @(posedge clk); wt = wt + 1;
            end
            if (u_tcb.snd_una_r[0] != s) tmo_vec[6] = 1'b1;
        end
    endtask
    ...
    task cmd_close;              // 0x06 CMD: cmd=1 close (fin_req[id])
        input [3:0] id;
        begin
            reg_write(8'h06, {28'd0, 4'd1, id});
        end
    endtask

    task cmd_abort;              // 0x06 CMD: cmd=2 abort (rst_req[id])
        input [3:0] id;
        begin
            reg_write(8'h06, {28'd0, 4'd2, id});
        end
    endtask

    task cfg_add;                // 8 词 ADD 记录 (slot0 四元组; 与 HLS cfg_stream 同格式)
        input [3:0]  slot;
        input [31:0] snxt;
        begin
            cfg_record(32'h0008_0000 | {28'b0, slot},   // w0 = (wscale=8<<16)|(cmd=0<<8)|slot
                       TB_PEER_IP, TB_MY_IP,
                       {TB_PEER_PORT, TB_MY_PORT},
                       TB_PEER_MAC[47:16],
                       {TB_PEER_MAC[15:0], 16'h4000},
                       PEER_ISN + 32'd1, snxt);
        end
    endtask

    task cfg_del;                // 8 词 DEL 记录 (cmd=1: CAM 清 + state=0 + ev_down)
        input [3:0] slot;
        begin
            cfg_record(32'h0000_0100 | {28'b0, slot}, 32'd0, 32'd0, 32'd0,
                       32'd0, 32'd0, 32'd0, 32'd0);
        end
    endtask

    task inj_fin;                // 对端 FIN 注入 (flags 0x11, 60B, 走慢路径)
        input [31:0] sq;
        input [31:0] ak;
        begin
            inj_arm(1'b0, 1'b1, sq, ak);
            repeat (2) @(posedge clk);
            wt = 0;
            while (inj_play && (wt < 4000)) begin @(posedge clk); wt = wt + 1; end
            if (inj_play) tmo_vec[7] = 1'b1;
    ...
    task ph_open;
        ...
        begin
            ph_id   = id;
            ph_base = base;
            ph_d0   = {16'b0, frm_seen};
            ph_f0   = {16'b0, frm_fin_n};
            ph_r0   = {16'b0, frm_rst_n};
            ph_e0   = ac_ev_down_c;
            ph_s0   = st0_n;
            ph_ab0  = pat_tx_bytes;
            ph_af0  = pat_tx_frames;
            ph_t0   = k;
            // PC 模型复位 (序号期望/高水位/失序标志)
            exp_seq = base;
            hi_wm   = 32'd0;
            hole    = 1'b0;
        end
    ...
    task ph_close;
        begin
            ph_d = {16'b0, (frm_seen  - ph_d0[15:0])};
            ph_f = {16'b0, (frm_fin_n - ph_f0[15:0])};
            ph_r = {16'b0, (frm_rst_n - ph_r0[15:0])};
            ph_e = ac_ev_down_c - ph_e0;
            $fwrite(fd, "P5PH %0d %08h %0d %0d %0d %0d %0d %0d\n",
                    ph_id, ph_base, ph_d, ph_f, ph_r, ph_e, ph_t0, k);
        end
    endtask

    task dump_ph_regs;
        begin
            dump_regp(8'h0A); dump_regp(8'h10); dump_regp(8'h11); dump_regp(8'h12);
            dump_regp(8'h13); dump_regp(8'h50); dump_regp(8'h90); dump_regp(8'h91);
            dump_regp(8'h93); dump_regp(8'h94); dump_regp(8'h97); dump_regp(8'h99);
            dump_regp(8'h9A);
        end
    endtask
    ...
    task close_main;
        begin
            ...
            inj_wnd = 16'h4000;
            bad_idx = 16'd0;
            rxdata_at = 32'hFFFFFFFF;      // 本门不注数据段 (RX 通路不出现在判据里)
            cfg_tdata = 32'd0; cfg_tvalid = 1'b0;
            ac_addr = 8'h00; ac_wr = 1'b0; ac_wdata = 32'd0;
            frm_fin_n = 16'd0; frm_rst_n = 16'd0; frm_oth_n = 16'd0;
            fin_drop_rem = 16'd0; fin_acked_n = 16'd0;
            fin_seq0 = 32'd0; fin_seq_last = 32'd0;
            st0_n = 16'd0; finreq_n = 16'd0; rstreq_n = 16'd0;
            tmo_vec = 32'd0; frm_idx = 0;
            ...
            fd = $fopen("resp_p5_udp_split_close.memh", "w");
            #200; rst_n = 1;
            repeat (400) @(posedge clk);
            $fwrite(fd, "P5CFG rto_lim=%0d fin_to_lim=%0d fin_grace=%0d rto_ticks=%0d tx_bytes=%0d nframes=%0d auto_close=%0d ph_n=%0d\n",
                    TB_RTO_LIM, TB_FIN_TO_LIM, TB_FIN_GRACE, TB_RTO_TICKS,
                    TB_TX_BYTES, PH_NFRAMES, TB_AUTO_CLOSE, 7'd7);
            $fwrite(fd, "P5START k=%0d\n", k);

            // ============ 阶段 A: 干净关闭 (判据 ①) ============
            ph_open(4'd1, PH_A_BASE);
            cfg_add(4'd0, PH_A_BASE);
            wait_dfr(PH_NFRAMES);          // 8 个数据帧上线
            wait_idle();                   // 数据全部被 ACK (FIN 的前置硬规则 D4)
            cmd_close(4'd0);               // 关闭请求 -> fin_req[0]
            wait_dff(16'd1);               // 我方 FIN 上线 (恰一个)
            wait_una_eq(fin_seq_last + 32'd1);  // 对端 ACK 掉 FIN (snd_una 越过)
            wait_tk(TB_RTO_TICKS * 32'd2); // 观察窗 2 x RTO: 不得有第二个 FIN
            ph_close();
            dump_ph_regs();
            cfg_del(4'd0);
            wait_tk(400);

            // ====== 阶段 B: FIN 丢失 -> RTO 重发 (判据 ②) + 关闭超时 (判据 ⑤) ======
            ph_open(4'd2, PH_B_BASE);
            cfg_add(4'd0, PH_B_BASE);
            wait_dfr(PH_NFRAMES);
            wait_idle();
            fin_drop_rem = 16'd1;          // 丢首个 FIN 的 ACK
            cmd_close(4'd0);
            wait_dff(16'd1);               // FIN#1 (未 ACK)
            wait_dff(16'd2);               // FIN#2 = RTO 重发 (逐字节一致)
            wait_dfrst(16'd1);             // 关闭超时到点 -> RST 真的发出
            wait_devdown(16'd1);           // CONN_DOWN 事件脉冲
            wait_state0(16'd1);            // state 被写 0 (RST 之后)
            wait_tk(TB_RTO_TICKS * 32'd2); // 观察窗: 不得有第三个 FIN / 第二个 RST
            ph_close();
            dump_ph_regs();                // 超时路径的配额归还 (DEL 之前读)
            cfg_del(4'd0);
            wait_tk(400);

            // ====== 阶段 D1: 同时关闭 (我方 FIN 先) — fast path 恰 1 个 FIN ======
            ph_open(4'd3, PH_D1_BASE);
            cfg_add(4'd0, PH_D1_BASE);
            wait_dfr(PH_NFRAMES);
            wait_idle();
            fin_drop_rem = 16'd1;          // 对端"同时"关闭: 我方 FIN 不被 ACK
            cmd_close(4'd0);
            wait_dff(16'd1);               // 我方 FIN (唯一一个)
            inj_fin(PEER_ISN + 32'd1, fin_seq_last + 32'd1);   // 对端 FIN 注入
            cfg_del(4'd0);                 // 模拟 HLS 收到 FIN 后拆连
            wait_tk(TB_RTO_TICKS * 32'd3); // 观察窗 3 x RTO: 死连接不得重发 FIN
            ph_close();                    // 期望 f = 1 (且此后不再增)
            dump_ph_regs();

            // ====== 阶段 D2: 同时关闭 (DEL 先到) — fast path 发 0 个 FIN ======
            ph_open(4'd4, PH_D2_BASE);
            cfg_add(4'd0, PH_D2_BASE);
            wait_dfr(PH_NFRAMES);
            wait_idle();
            inj_fin(PEER_ISN + 32'd1, PH_D2_BASE);   // 对端 FIN (ack = 我方 snd_nxt)
            cfg_del(4'd0);                 // 连接先被拆 (state=0 + CAM 清)
            wait_tk(TB_RTO_TICKS);
            cmd_close(4'd0);               // 关闭请求落在已拆连接上
            wait_tk(TB_RTO_TICKS * 32'd2);
            ph_close();                    // 期望 f = 0
            dump_ph_regs();

            // ====== 阶段 F: 新连接拿到非零 rcv_wnd (判据 ⑤ 尾) ======
            ph_open(4'd5, PH_F_BASE);
            cfg_add(4'd0, PH_F_BASE);
            wait_dfr(PH_NFRAMES);
            wait_idle();
            ph_close();
            dump_ph_regs();                // winq[0] != 0 / pool == 0
            inj_fin(PEER_ISN + 32'd1, PH_F_BASE + TB_TX_BYTES);
            cfg_del(4'd0);
            wait_tk(400);

            // ====== 阶段 C: abort -> RST + state=0 (判据 ③, 末阶段) ======
            ph_open(4'd6, PH_C_BASE);
            cfg_add(4'd0, PH_C_BASE);
            wait_dfr(16'd1);               // 数据在飞 (G3 fence 才有可观测性)
            cmd_abort(4'd0);               // abort 请求 -> rst_req[0]
            wait_dfrst(16'd1);             // RST 真的发出
            wait_state0(16'd1);            // state 被写 0
            wait_tk(TB_RTO_TICKS * 32'd2); // 观察窗: RST 之后不得再起数据帧
            ph_close();
            dump_ph_regs();
            $fwrite(fd, "P5APPTX %0d %0d %0d %0d %0d %0d %0d\n",
                    6'd6, pat_tx_bytes - ph_ab0, pat_tx_frames - ph_af0,
                    app2_tvalid, app2_tready, pat_active, pat_done);

            // ====== 阶段 G: abort fence 探针 (RST 早于数据 → 非空判据) ======
            // 阶段 C 的 RST 被 scan_now 饥饿推迟到"app 数据流结束"附近 ⇒ 那里
            // "RST 后不起数据帧" 为空判据。本阶段在**数据未上线前**就 abort:
            // RST 必须早于 app 的第 2 帧 ⇒ 剩余 23 帧被 G3 fence (tx_blk) 挡住,
            // 线上数据帧数 << PH_NFRAMES 且 app 停在推帧状态 (tvalid=1/tready=0)。
            ph_open(4'd7, PH_G_BASE);
            cfg_add(4'd0, PH_G_BASE);
            cmd_abort(4'd0);
            wait_dfrst(16'd1);
            wait_state0(16'd1);
            wait_tk(TB_RTO_TICKS * 32'd2);
            ...
            dump_ph_regs();
            $fwrite(fd, "P5APPTX %0d %0d %0d %0d %0d %0d %0d\n",
                    7'd7, pat_tx_bytes - ph_ab0, pat_tx_frames - ph_af0,
                    app2_tvalid, app2_tready, pat_active, pat_done);
            ...
            $fwrite(fd, "P5CLOSE end k=%0d tmo=%08h fin=%0d rst=%0d oth=%0d finack=%0d st0=%0d finreq=%0d rstreq=%0d evdown=%0d\n",
                    k, tmo_vec, frm_fin_n, frm_rst_n, frm_oth_n, fin_acked_n,
                    st0_n, finreq_n, rstreq_n, ac_ev_down_c);
            $fclose(fd);
            $display("P5CLOSE DONE k=%0d fin=%0d rst=%0d tmo=%08h", k, frm_fin_n,
                     frm_rst_n, tmo_vec);
            $finish;
        end
    endtask
`endif
    ...
    initial begin
`ifdef P5_CLOSE
        // P5c-T4: close 门 = 完全自包含的驱动 (自带初始化/落盘/收尾/$finish),
        // 默认路径的语句一条都不执行 (也不会被本分支改动)
        close_main;
`else
        clk = 0; rst_n = 0;
        inj_wnd = 16'h4000;
...
`else
        fd = $fopen("resp_p5_udp_split.memh", "w");
`endif
        cfg_tdata = 32'd0; cfg_tvalid = 1'b0;
        ac_addr = 8'h00; ac_wr = 1'b0; ac_wdata = 32'd0;
        #200; rst_n = 1;
