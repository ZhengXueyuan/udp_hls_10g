...
// 校验: ①复位后 run=0 期间线保持空闲高; ②run 后第 1 行 445 字节 ASCII 逐字节
...
// 不影响本行); ④末尾 CR LF。
//
// P4b-7-P6 扩展: 行 443 可打印字符 (117→146→182→216→301→358→366→443; 快照行
//   CR 在 443 / LF 在 444, SNAP_M1=444), RX 侧冻结位点字段
...
//   行 k 字节 j = 边存址 (rptr-32+8k+j)&8191 的 tlast (P4c AW=13), 打印 00/01。
// TB 侧模型: 边存镜像 tlm[0:8191] (P4c AW=13; bit8 = (a%13==5), 周期 13 与 8/64 互素 →
...
// 覆盖: 第 1/2 轮 run=1&tr_run=0 (快照 + 8 TL, 基址 = mid-line 改写后的 1456
// → 基址 1436, 高位址 >4095 专测 13 位不截断); 第 3 轮 run=1&tr_run=1 (快照 +
// 4 TR + 8 TL), 且 TL 前把 rpt 改为 1FF0 → 基址 1FD0, 读 1FD0..200F 跨 8192
// 回卷 (13 位域回卷专测); 第 4 轮 run=0&tr_run=1 → 无 TL
...
//
// P4b-7-P6-P6b WL 扩展 (线上帧长, 行宽 360 → 368): 快照行追加
//   " WL=%04X" (8 字符, pos 358..365; P4c 前为行末, 现其后接 diag18/TRU,
//   CR 在 443 / LF 在 444), 输入 rx_wire_last =
...
//   漏首字符都会失配); CR/LF 位置同步后移 (366/367 → P4c 后 443/444), 行宽/
//   SNAP_M1 错则整行失配。
//
// P4c 扩展 (AW=13 加宽 + diag18/TRU 字段同步, 行宽 368 → 445):
//   ① WPT/RPT 加宽为 4 位 hex (wptr/rptr[12:0] 值域 0..1FFF): 字段仍 8 字符
//      (pos 101..108 / 109..116), 加宽靠吃掉原前导分隔空格 (WPT 与 FFE 末位
//      字符直接相邻)。行 1 = 1ABC/1123, 行 2 = 1DEF/1456 (第 3 轮后 rpt =
//      1FF0) — 顶位 nibble 非 0, 旧 %03X 截断即逐字节失配。
//   ② diag18 字段 (pos 366..428: SC/SD/SF/SP 32 位 + SV 22 位 + HR 1 位) +
//      pos 429 占位空格 + TRU (430..442) 与其它实时字段同行首重采语义
//      (行 1 初值 / 行 2 mid-line 改写值)。
//   ③ TL 读址加宽 13 位: exp_tl 掩码 13'h1FFF + 镜像 tlm[0:8191]; 第 1/2 轮
//      基址 1436 (高位址不被截断), 第 3/5 轮基址 1FD0 (跨 8192 回卷) — 12 位
//      截断实现会把 >4095 的址折回低区, 周期 13 图案下必逐字节失配。
module tb_uart_dbg;
    reg clk = 0;
    ...
    reg  [12:0] wpt, rpt;                    // fifo wptr / rptr (P4c AW=13)
    ...
    // P4c: diag18 慢路径/HLS 存活字段 + TRU (快照行尾 366..442)
    reg  [31:0] sc, sd;                      // srx_commit / srx_drop (slow_rx_adp)
    reg  [31:0] sf, sp;                      // stx_frames / stx_purge (slow_tx_adp)
    reg  [21:0] srv;                         // starv (看门狗饥饿累计, 22 位)
    reg         hr;                          // hls_rst_n 实时 (0 = 复位中)
    reg  [31:0] tru;                         // rx_drop_trunc (trunc 修复计数)
    wire        txd;
    // P4b-7-P6 TL: 边存读口 (DUT 出址, TB 镜像数组组合出值) + 期望基址
    reg  [8:0]  tlm [0:8191];                // 边存镜像 (P4c AW=13; bit8 = tlast,
                                             //   a%13==5 — 周期 13 与 8/64 互素)
    wire [12:0] tl_addr;                     // DUT dbg_rd_addr (读引擎址, P4c 13 位)
    wire [8:0]  tl_side = tlm[tl_addr];      // 组合读回 DUT dbg_rd_side
    reg  [12:0] rpt_exp;                     // TB 侧 rpt 基准 (exp_tl 用, P4c 13 位)
    ...
    dbg_line_tx #(.BIT_LAST(BITL), .GAP_LAST(GAPL)) uut (
        ...
        .rx_wire_last(wl),
        .srx_commit(sc), .srx_drop(sd), .stx_frames(sf), .stx_purge(sp),
        .starv(srv), .hls_rst(hr), .rx_drop_trunc(tru),
    ...
    // ---- TL 行期望字符 (独立算式: 基址 rpt_exp-32 (13 位回卷) + 8k+j → 镜像) ----
    function [7:0] exp_tl;
        ...
        begin
            if      (pos == 9'd0)  exp_tl = 8'h54;     // 'T'
                ...
                u   = pos - 9'd3;                  // 0..15
                j   = u / 2;                       // 字节号 0..7
                aad = (rpt_exp - 13'd32 + k * 8 + j) & 13'h1FFF;
    ...
    // 独立; P4c: 443 可打印字符 + CR LF, CR/LF 在 443/444):
    ...
    //   FFE 10→01  WPT 1ABC→1DEF  RPT 1123→1456 (P4c 4 位 hex, 字段无前导空格;
    //   第 3 轮后 rpt 再改 1FF0 → 第 5 轮快照行 RPT 字段 = 1FF0)
    ...
    //   P4c diag18/TRU (pos 366..442): SC/SD/SF/SP 00000021..24 → 1A2B3C4D/
    //   5E6F7081/9ABCDEF0/FEDCBA98, SV 012345→3A5C7E (22 位), HR 1→0,
    //   TRU 00000123→89ABCDEF — 与其它实时字段同行首重采语义。
    // 注: xsim 把字符串字面量里的 \r 存成 0x72('r') 而非 0x0D, 故期望串不含
    // 转义, 只含 443 个可打印字符; 行尾 CR LF 单独用 0x0D/0x0A 显式核对。
    // 行尾三站词计数: 行 1 = 初值 (mw/cwi/cwo/rw = 1/2/3/4, wc=1), 行 2 =
    // mid-line 改写值 (11/22/33/44, wc=5) — 与其它实时字段同行首锁存语义。
    reg [3543:0] exp1 = "NX=FEDCBA98 UA=01234567 WN=8BAD WS=F ST=5 W=1011 I=1001 E=2FFE TXST=3 RXST=1 ACC=1 EMV=0 EST=1 FFE=10WPT=1ABCRPT=1123 PF=1 TV=0 PV=1 SV=1 PLEN=22F TW=11111111 TF=22222222 TI=44444444 PW=0A5 PR=0C3 PFL=1 PEM=0 PLN=123 RXPL=0123 RXPC=0456 RXT=0789 DROPS=0000000A/0000000B/0000000C/0000000D PASS=0000000E RXTR=0 MW=00000001 CW=00000002/00000003 RW=00000004 WC=1 WL=0042 SC=00000021 SD=00000022 SF=00000023 SP=00000024 SV=012345 HR=1  TRU=00000123";
    reg [3543:0] exp2 = "NX=FEDCBA98 UA=01234567 WN=8BAD WS=F ST=5 W=1011 I=1001 E=2FFE TXST=7 RXST=2 ACC=0 EMV=1 EST=0 FFE=01WPT=1DEFRPT=1456 PF=0 TV=1 PV=0 SV=0 PLEN=345 TW=12345678 TF=9ABCDEF0 TI=0F0F0F0F PW=1B6 PR=1D4 PFL=0 PEM=1 PLN=4AB RXPL=ABCD RXPC=CDEF RXT=EF01 DROPS=11223344/55667788/99AABBCC/DDEEFF00 PASS=0F1E2D3C RXTR=0 MW=00000011 CW=00000022/00000033 RW=00000044 WC=5 WL=05EA SC=1A2B3C4D SD=5E6F7081 SF=9ABCDEF0 SP=FEDCBA98 SV=3A5C7E HR=0  TRU=89ABCDEF";

    // ---- mid-line 输入改写 (第 1 行发送途中; 行首快照不应受影响) ----
    initial begin
        while (!run) @(posedge clk);        // 等主流程置 run
        repeat(25_000) @(posedge clk);      // 第 1 行 (445 字 ~358k 拍) 中段
        ts  = 3'd7;
        rxs = 3'd2; acc = 1'b0; emv = 1'b1;
        est = 3'd0; ff = 1'b0; fe = 1'b1;
        wpt = 13'h1DEF; rpt = 13'h1456;     // 只应影响第 2 行 (P4c: 13 位, 顶位非 0
                                            //  → 旧 %03X 截断/漏重采都逐字节可见)
        ...
        rpt_exp = 13'h1456;                 // 第 1/2 轮 TL 期望基址源 (= rpt-32)
        ...
        sc = 32'h1A2B3C4D; sd = 32'h5E6F7081;   // P4c diag18: 全 nibble 互异值
        sf = 32'h9ABCDEF0; sp = 32'hFEDCBA98;
        srv = 22'h3A5C7E; hr = 1'b0; tru = 32'h89ABCDEF;
    end
    ...
    initial begin
        ...
        wpt = 13'h1ABC; rpt = 13'h1123;      // P4c: 13 位 (顶位 nibble = 1)
        ...
        sc = 32'h00000021; sd = 32'h00000022;   // P4c diag18 行 1 初值
        sf = 32'h00000023; sp = 32'h00000024;
        srv = 22'h012345; hr = 1'b1; tru = 32'h00000123;
        errs = 0; idlec = 0;
        ...
        // P4c: 13 位址空间 (0..8191) 全填, 高位址 (>=4096) 亦覆盖
        for (aa = 0; aa < 8192; aa = aa + 1) tlm[aa] = {((aa % 13) == 5), 8'b0};
        rpt_exp = 13'h1456;              // TL 期望基址源 = mid-line 改写后的 rpt
                                         // (改写 ~25k 拍 << 快照行末 243k, 故第 1
                                         //  轮 TL 突发时 DUT rpt 已是 1456 → 基址
                                         //  1436 > 4095: 12 位截断即失配)
        ...
        for (k = 0; k < 445; k = k + 1) begin
            ...
            if (k != 444) repeat(2 * PER + 1) @(posedge clk);
        end

        for (i = 0; i < 443; i = i + 1) eb[i] = exp1[3543 - 8 * i -: 8];
        $display("L1 decoded: ");
        for (i = 0; i < 445; i = i + 1) $write("%c", g1[i]);
        $write("\n");
        for (i = 0; i < 443; i = i + 1)
            if (g1[i] !== eb[i]) begin
        ...
        if ((g1[443] !== 8'h0D) || (g1[444] !== 8'h0A)) begin
            $display("FAIL: L1 行尾 got %02X %02X (expect 0D 0A)", g1[443], g1[444]);
            errs = errs + 1;
        end
        $display("L1 last two bytes: %02X %02X (expect 0D 0A)", g1[443], g1[444]);

        // ===== 第 1 轮 8 行 TL (run=1 & tr_run=0: 快照行后直接 TL 突发) =====
        // 基址 = rpt-32 = 1456-32 = 1436 (13 位域, 高位址 >4095 不截断);
        // 行 k(=lno-5) 字节 j = 址 1436+8k+j
        ...
        // FFE=01 WPT=1DEF RPT=1456 PF=0 TV=1 PV=0 SV=0 PLEN=345
        ...
        // SC/SD/SF/SP=1A2B3C4D/5E6F7081/9ABCDEF0/FEDCBA98 SV=3A5C7E HR=0
        // TRU=89ABCDEF (P4c diag18/TRU: 行首重采))
        for (i = 0; i < 443; i = i + 1) eb[i] = exp2[3543 - 8 * i -: 8];
        $display("L2 decoded: ");
        for (i = 0; i < 445; i = i + 1) $write("%c", g2[i]);
        $write("\n");
        for (i = 0; i < 443; i = i + 1)
            if (g2[i] !== eb[i]) begin
        ...
        if ((g2[443] !== 8'h0D) || (g2[444] !== 8'h0A)) begin
            $display("FAIL: L2 行尾 got %02X %02X (expect 0D 0A)", g2[443], g2[444]);
            errs = errs + 1;
        end
        ...
        if ((g1[443] !== 8'h0D) || (g1[444] !== 8'h0A)) begin
            $display("FAIL: L3 行尾 got %02X %02X (expect 0D 0A)", g1[443], g1[444]);
            errs = errs + 1;
        end
        // -- 第 3 轮 TL 基址改为 rpt=1FF0 (基址 1FF0-32 = 1FD0, 13 位回卷专测:
        // 读 1FD0..200F 跨 8192 回卷, 且址 >4095 = 12 位截断判别) —
        // 改点在快照行之后 (TR 行不用 rpt; TL 基址在 TR4 行末才锁存, 故生效)
        rpt = 13'h1FF0; rpt_exp = 13'h1FF0;
        ...
        // ===== 第 3 轮 8 行 TL (TR4 后, 基址 1FD0 跨 8192 回卷) =====
        for (l = 0; l < 8; l = l + 1) chk_tl(l[3:0]);
        $display("L3 TL burst done (base 1FD0 wrap@8192, 8 lines ok)");
        ...
        decode_line(445);
        for (i = 0; i < 443; i = i + 1)
            eb[i] = exp2[3543 - 8 * i -: 8];
        // 本轮输入 = 第 3 轮中途改写后的稳态值: rpt 已是 1FF0 (exp2 里是 1456,
        // 那是第 3 轮快照行拍上的值, 之后从未还原) → RPT 字段 (char 113..116,
        // P4c: 4 位 hex "1FF0") 覆盖; 行首重采 rxt_run=1 → RXTR 字段 (char 307) = '1'
        eb[113] = 8'h31; eb[114] = 8'h46; eb[115] = 8'h46; eb[116] = 8'h30; // RPT=1FF0
        eb[307] = 8'h31;                                     // RXTR=1
        ...
        if ((g1[443] !== 8'h0D) || (g1[444] !== 8'h0A)) begin
            $display("FAIL: L5 行尾 got %02X %02X (expect 0D 0A)", g1[443], g1[444]);
            errs = errs + 1;
        end
        // 8 行 TL (rpt=1FF0 未再变 → 与第 3 轮同基址 1FD0)
        for (l = 0; l < 8; l = l + 1) chk_tl(l[3:0]);
