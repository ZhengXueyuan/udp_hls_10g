`timescale 1ns/1ps
// TCP echo 全链 TB: GMII -> mac_rx_64 -> tcp_rx -> tcp_echo -> tcp_tx_frame -> mac_tx_64 -> GMII。
// tcp_synp 接 tcp_rx SYN sideband (conn0 握手建立); ACK 请求二选一 (synp 优先, ack_syn=1);
// CAM 配置口二选一 (TB 配置阶段优先, 其次 synp); TCB 更新仲裁 tx > rx > cfg 级 (TB 配置|synp)。
// 与 tb_tcp_chain 的差异: app 脚本驱动换成 tcp_echo 载荷环回 (conn_id 随帧排队)。
// RX 帧间由生成器放大间距 (gap 字节) — TX 每数据帧要发 ACK+echo 两帧, 近距离背靠背
// 会让 mac 8 深 FIFO 满丢帧; 大间距下 tcp_rx 载荷口 tready 恒 1 (echo frame_fifo 不满)。
// 捕获 (resp_tcp_echo.memh): 每拍 "%02h %d" (gmii_txd, tx_en) + 事件行
//   FEND k ferr / ACK k id val (含 synp 的 SYN+ACK 请求) / SYNP 对端握手字段。
// 末尾 STATS7 (tcp_rx) + STATS_TX (frames bytes ack ackdrop) + STATS_ECO (echo drop_crc)
// + CAMF (CAM 条目 0) + TCBF (conn0/1 全 6 字段)。
module tb_tcp_echo;
    ...
    initial begin
        ...
        $fclose(fd);
        $display("DONE rx(pass=%0d nm=%0d ip=%0d crc=%0d seq=%0d ack=%0d by=%0d) tx(fr=%0d by=%0d ack=%0d drop=%0d) eco(echo=%0d drop=%0d)",
                 rx_stat_pass, rx_stat_nonmatch, rx_stat_ipcsum, rx_stat_crc,
                 rx_stat_seq, rx_stat_ack, rx_stat_bytes,
                 tx_stat_frames, tx_stat_bytes, tx_stat_ack, tx_stat_ack_drop,
                 eco_stat_echo, eco_stat_drop_crc);
