...
// P4c: ring 16KB/conn -> 64KB/conn, 字节偏移 [15:0] (模 2^16), 末端 = 0xFFFF。
// 组 A: seq 偏移 0..7 x n 1..8 (偶/奇 ring 字); B: LFSR 伪随机 2000 写后回读;
// C: ring 末端回卷; D: 双连接交错; E: 读延迟恰 2 拍 (+ 空闲保持); F: 复位后一致性。
// P4c 步骤 3 (2026-09-12): 读地址入口寄存 (rd_en 门控) => 读延迟 1 拍 -> 2 拍;
//   调用方 (tcp_tx_frame S_RING) 写窗口后移 1 拍, 组 G 已按 2 拍重写。
...
module tb_retx_ram;
    ...
    // ---- 写: 1 拍使能并更新参考模型 ----
    task t_write(input [3:0] c, input [15:0] s, input [3:0] n, input [63:0] d);
        integer k;
    begin
        @(posedge clk); #1;
        wr_en = 1; w_conn = c; w_seq = s; w_n = n; w_data = d;
        for (k = 0; k < n; k = k + 1)
            ref[c][(s + k) & 16'hFFFF] = (d >> (8 * (7 - k))) & 8'hFF;
        @(posedge clk); #1;
        wr_en = 0;
    end
    endtask

    // ---- 读: rd_en 1 拍, 此后第 2 个 posedge 后采样 r_data ----
    // P4c 步骤 3: 地址于 rd_en 拍末入 ra_e_r, 次拍读出 (rd_d1) => 数据第 2 拍有效。
    task t_read(input [3:0] c, input [15:0] s, output [63:0] v);
    begin
        @(posedge clk); #1;
    ...
    task t_exp(input [3:0] c, input [15:0] s, output [63:0] e);
        integer k;
    begin
        e = 64'h0;
        for (k = 0; k < 8; k = k + 1) e = (e << 8) | ref[c][(s + k) & 16'hFFFF];
    end
    endtask

    // ---- 校验读 == ref, 失配即 $fatal ----
    task t_verify(input integer gid, input [3:0] c, input [15:0] s);
        reg [63:0] v, e;
    begin
        t_read(c, s, v);
        t_exp(c, s, e);
    ...
    initial begin
        ...
        for (it = 0; it < 2000; it = it + 1) begin
            lfsr = nxt(lfsr);
            w_conn = lfsr[17:14];
            w_seq  = ((it % 40) == 0) ? (16'hFFF0 | lfsr[3:0]) : lfsr[15:0];
            w_n    = 1 + lfsr[22:20];
        ...
        // ================= 组 C: ring 末端回卷 =================
        dat = 64'h11_22_33_44_55_66_77_88; t_write(0, 16'hFFF8, 8, dat);   // 字 8191 整字
        dat = 64'hAA_BB_CC_DD_EE_FF_01_02; t_write(0, 16'hFFFC, 8, dat);   // 跨末端溢出
        dat = 64'h0F_1E_2D_3C_4B_5A_69_78; t_write(0, 16'hFFFF, 5, dat);   // o=7 溢到 0..3
        t_verify(2, 0, 16'hFFF8);
        t_verify(2, 0, 16'hFFFC);
        t_verify(2, 0, 16'hFFFF);
        t_verify(2, 0, 16'hFFFD);
        t_verify(2, 0, 0);               // 溢出落点 (字 0 已被 5B 写覆盖低 4 字节)
        dat = 64'hDE_AD_BE_EF_CA_FE_BA_BE; t_write(1, 16'hFFFF, 8, dat);   // 他 conn
        t_verify(2, 1, 16'hFFFF);
        $display("GRP C (ring wrap 65535)            : PASS");

        // ================= 组 D: 双连接交错 =================
        dat = 64'h01_23_45_67_89_AB_CD_EF; t_write(3, 9, 4, dat);
        dat = 64'hFE_DC_BA_98_76_54_32_10; t_write(11, 65532, 8, dat);
        t_verify(3, 3, 9);
        t_verify(3, 11, 65532);
        dat = 64'h12_34_56_78_9A_BC_DE_F0; t_write(3, 65535, 6, dat);
        ...
        // ================= 组 E: 读延迟恰 2 拍 + 空闲保持 =================
        // 语义 (P4c 步骤 3, 读地址入口寄存): rd_en + 稳定 r_seq 于拍内保持, 地址于
        // 该拍末入 ra_e_r、次拍读出 => 第 2 个 posedge 后 r_data 有效; 第 1 个沿后仍
        // 为旧读数 (延迟非 1), 撤销 rd_en 后保持读数 (非每拍重读)。
        ...
        @(posedge clk); #1;
        v1 = r_data;                            // 1 拍后: 仍非 B (延迟非 1)
        if (v1 === exp)
            $fatal(1, "GRP4 FAIL: latency<2 conn=0 seq=400 got=%016h", v1);
        @(posedge clk); #1;
        v1 = r_data;                            // 恰 2 拍后有效
        if (v1 !== exp)
            $fatal(1, "GRP4 FAIL: latency!=2 conn=0 seq=400 exp=%016h got=%016h", exp, v1);
        rd_en = 0; r_seq = 0;
        @(posedge clk); #1;
        ...
        $display("GRP E (latency exactly 2 cyc)      : PASS");

        // ================= 组 F: 复位后一致性 =================
        dat = 64'hC0_C1_C2_C3_C4_C5_C6_C7; t_write(7, 123, 8, dat);
        dat = 64'hD0_D1_D2_D3_D4_D5_D6_D7; t_write(7, 65532, 8, dat);
        ...
        // ================= 组 G: 流式连续读 (S_RING 用法) =================
        // 写 9 个模式于 conn2 seq=8k (k=0..8), 然后连续读 (rd_en 恒 1,
        // r_seq 每拍推进 8, 无空闲) — 正是 retx 回放 (S_RING) 的读模式:
        // S_RING 每拍发 rd_en+新地址 (预读), 但数据晚 2 拍出 (P4c 步骤 3 地址入口
        // 寄存), 故拍 C(j) 消费的是 C(j-2) 呈现地址的字 — 调用方写窗口整体后移
        // 1 拍。每拍两采样:
        // (a) 拍起点: 输出 = 2 拍前地址的字 — 延迟语义;
        // (b) 同拍 r_seq 已推进到下一字: 输出必须不变 (q/r_sel 均为寄存器) —
        //     组合 r_sel 时选择超前 q 1 拍 → 此处全错位 (静态读测不到)。
        ...
        @(posedge clk); #1;
        rd_en = 1; r_conn = 2; r_seq = 0;   // C0: 呈现地址 0
        @(posedge clk); #1;
        r_seq = 8;                          // C1: 呈现地址 8 (C0 的 0 已入 ra_e_r)
        for (k = 0; k < 8; k = k + 1) begin
            @(posedge clk); #1;             // C(k+2) 起点: 输出 = C(k) 呈现地址 8k 的字
            v1 = r_data;
            t_exp(2, 8 * k, exp);
            if (v1 !== exp)
                $fatal(1, "GRP6 FAIL: stream k=%0d addr=%0d exp=%016h got=%016h",
                       k, 8 * k, exp, v1);
            r_seq = 8 * (k + 2);            // 同拍推进: 输出必须仍为 8k 的字
            #1;
            v1 = r_data;
            if (v1 !== exp)
        ...
        rd_en = 0; r_seq = 0;               // 撤读: 末读数 (在飞地址 64) 须保持
        repeat (2) begin
            @(posedge clk); #1;
            v1 = r_data;
            t_exp(2, 8 * 8, exp);
